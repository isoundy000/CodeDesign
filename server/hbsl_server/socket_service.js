...
var GameStatu = require('./define').GameStatu;
var io = null;
exports.start = function(config,mgr){
	...
	io.sockets.on('connection',function(socket){
		...
		socket.on('ready',function(data){
			...
			//如果是游戏开始状态，则不可以准备
			if(socket.gameMgr.getGameStatu(userId) === GameStatu.GAME_START){
				return;
			}
			socket.gameMgr.setReady(userId);
		});
	...
	console.log("lr server is listening on " + config.CLIENT_PORT);	
};