var ddzRoomDB = require('../DB/managers/ddzRoomDBMgr');
...
exports.createRoom = function(creator,roomConf,gems,ip,port,callback){
	...
	var fnCreate = function(){
		...
		if(rooms[roomId] != null || creatingRooms[roomId] != null){
			...
			ddzRoomDB.is_room_exist(roomId, function(ret) {
				...
				if(ret){
					...
					ddzRoomDB.create_room(roomInfo.id,roomInfo.conf,ip,port,createTime,function(uuid){
						delete creatingRooms[roomId];
						if(uuid != null){
							roomInfo.uuid = uuid;
                            rooms[roomId] = roomInfo;
...
exports.enterRoom = function(params,callback){
	...
	var fnTakeSeat = function(room){
		...
		for(var i = 0; i < 3; ++i){
			...
			if(seat.userId <= 0){
				...
				ddzRoomDB.update_seat_info(roomId,i,seat.userId,"",seat.name);
				//正常
				return 0;
			}
		}
	...
	if(room){
		...
		ddzRoomDB.get_room_data(roomId,function(dbdata){
			if(dbdata == null){
				//找不到房间
				callback(2);
			}
...
exports.exitRoom = function(userId){
	...
	ddzRoomDB.set_room_id_of_user(userId,null);
