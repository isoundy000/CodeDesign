...
class CLauncher{
    ...
    saveLauncherData(timeoutSecond:number=5000):void{
        if (cc.sys.isNative && !Util.isInvalid(this._launcherData)){
            ...
            cc.sys.localStorage.setItem('launcherData', JSON.stringify(launcherData));
        }
    };
    ...
    public getLauncherData():any{
        let _data = null;
        if (cc.sys.isNative){
            _data = cc.sys.localStorage.getItem('launcherData');
            if (!Util.isInvalid(_data)) {
                cc.sys.localStorage.removeItem('launcherData');
                try {
                    _data = JSON.parse(_data);
                    if (!Util.isInvalid(_data) && (new Date().getTime() < _data.timeout))
                        return _data.data;
                } catch (error) {
                    cc.error(error);
                }
            ...
            if (Util.isEmptyStr(_str)) _str = userData;
            //_data = DES.decodeCBC(_str);
            _data = 'GolangLtdDT?'+encodeURI('Protocol=8&Protocol2=1&login_os='+Util.getRunEnvironment());
        }
        return _data;
    };
};
